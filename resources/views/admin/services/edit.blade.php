...
@section('admin-content')
    <h2 class="text-center text-green mt-5">Изменить услугу "{{ $service->name }}"</h2>
    <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('service_category_id') is-invalid @enderror" id="category" name="service_category_id">
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" @if($category->id == old('service_category_id', $service->service_category_id)) selected @endif>{{ $category->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <img src="{{ asset('/storage/' . $service->image) }}" alt="{{ $service->name }}" width="400">
        </div>
        <div class="mb-3">
